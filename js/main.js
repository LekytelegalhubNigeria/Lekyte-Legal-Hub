...
const SUPABASE_ANON_KEY =
  "sb_publishable_OWoSsbLc4kmQ6bUTyEnSww_b4Qbhi3z";
...
window.shareProperty = function (id = "") {
  ...
  // ALWAYS use "property" (important fix)
  const url = id
    ? `${baseUrl}?property=${encodeURIComponent(id)}`
    : baseUrl;

  const text = `Check this verified property on Lekyte Legal Hub: ${url}`;
...
window.handleSignUp = async function (e) {
  ...
  try {
    const { data, error } = await supabaseClient.auth.signUp({
      ...
      password: password,
      options: {
        emailRedirectTo: "https://lekytelegalhubnigeria.github.io/Lekyte-Legal-Hub/"
      }
    });
    ...
    if (user) {
		const email = form.email.value.trim();
const fullName = document.getElementById('signupAgentName')?.value 
              || document.getElementById('signupAgentNameModal')?.value;
const phone = document.getElementById('signupAgentPhone')?.value 
           || document.getElementById('signupAgentPhoneModal')?.value;
const cac = document.getElementById('signupAgentCAC')?.value 
         || document.getElementById('signupAgentCACModal')?.value;
const esv = document.getElementById('signupAgentESV')?.value 
         || document.getElementById('signupAgentESVModal')?.value;
const state = document.getElementById('signupAgentState')?.value 
           || document.getElementById('signupAgentStateModal')?.value;
